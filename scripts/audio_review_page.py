...
for base, cands in targets.items():
    cells = [f'<label><input type="radio" name="{base}" value="old" checked> actual<br><audio controls preload="none" src="../../RiskAI/Assets/RiskAI/Resources/Audio/{base}.wav"></audio></label>']
    for c in cands:
...
<h1>Riesgus · revisión de sonidos</h1><p>Elige una opción por efecto y, si quieres, añade notas. Pasa el ratón sobre un candidato para ver su prompt. Luego pulsa el botón y pega el texto en el chat.</p>
...
<script>function out(){{const r={{pick:{{}},notes:{{}},general:document.getElementById('g').value}};document.querySelectorAll('input[type=radio]:checked').forEach(i=>r.pick[i.name]=i.value);document.querySelectorAll('input.n').forEach(i=>{{if(i.value.trim())r.notes[i.dataset.k]=i.value.trim()}});const t=JSON.stringify(r);document.getElementById('o').value=t;navigator.clipboard&&navigator.clipboard.writeText(t)}}</script>'''

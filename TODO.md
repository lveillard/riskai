...
- [x] Suavizar de forma acotada las esquinas de costa de Europe/NewWorld (`61f386d`): desplazamiento máximo de 0,2 celda (0,512 m), radios protegidos de 9 m en ciudades y 24 m en puertos, sin añadir triángulos. Render, agua, collider y consultas CPU comparten la deformación; alturas, flags y conectividad fuente conservados. Seguimiento: 10 EditMode y 10 PlayMode aprobados. No equivale a remodelar toda la costa. [Detalle](docs/VALIDATION-v0.22-FOLLOWUP.md).
- [x] Caballero con trote articulado, transición suave, fase continua y lanza ligada al golpe real; secuencia de 72 fotogramas con movimiento, pausa y ataque.
- [x] Reducir coste del minimapa (`2a9192b`): una capa reutilizada de marcadores a 10 Hz, conservando interacción, colores y cámara. Siete regresiones aprobadas; Web con minimapa visible mantiene 900 unidades durante 90 s y aplica 27.000 órdenes sin rechazos. Media observada 56,63 ms/frame frente a 85,92 anteriores, con carga externa variable: no es un A/B limpio.
- [x] Exportar Windows/Web finales y comprobar en navegador entrada, cámara, menús, ranking y NewWorld vertical. 52 casos Unity distintos aprobados en v0.22 y 12 Python; dispositivo ARM físico pendiente.
- [x] Sonda reproducible de 900 unidades: comprobación funcional de 30 s, todas movidas, 9000 órdenes aplicadas, cero rechazadas/pendientes. Compilación externa presente; no es validación limpia de rendimiento.
- [ ] Completar comparación limpia 500/1000/2000 y confirmar el candidato en partida avanzada. Las pasadas 500 y 1000 mantuvieron 900 unidades durante 90 s sin contención; repetir 2000 por compilación Rust externa. La preparación avanzada corregida completó 900 s simulados y 90 s medidos (942→454 unidades, 1.291 órdenes y cero rechazos), con carga externa. Falta repetirla sin contención. El presupuesto permanece en 500. [Seguimiento](docs/VALIDATION-v0.22-FOLLOWUP.md).
- [ ] Alcanzar mayor fluidez en Web con 900 unidades: la pasada final bajo carga externa sigue por debajo de 30 FPS. Repetir sin compiladores antes de atribuir el coste restante a simulación, motor, presentación o navegador.
- [ ] Validación física ARM/tablet y carga de combate sostenida; no cubiertas por el escenario aislado de navegación.

[Pruebas, capturas y límites v0.22](docs/VALIDATION-v0.22.md).
...
- [x] Eliminar interferencia entre círculo de selección, hover y círculo del guardián naval; mantener el barco anclado al disparar. Corregido y comprobado en v0.22; anclas terrestre/atraque conservadas. [Evidencia](docs/VALIDATION-v0.22.md).
...
- [x] Entrenamiento con luz visible saliendo de la puerta y umbral, compartida entre edificios/colas. Implementado e inspeccionado en v0.22, con luz por encima de los escalones.
...
- [x] Costa más natural: v0.22 completa clasificación compartida, materiales y agua somera; `61f386d` añade suavizado geométrico acotado en Europe/NewWorld, coherente con agua, collider y consultas CPU. Ciudades, islas, muelles y campo de clasificación protegidos; UV1 conservado y coste incluido en `StartupMetrics`/fase `terrain`. [Alcance y pruebas](docs/VALIDATION-v0.22-FOLLOWUP.md).
...
- [x] Suavizar de forma acotada la silueta escalonada de la costa importada sin contradecir puertos, islas y pasos navegables. Implementado y probado en `61f386d`; no supone una reconstrucción completa de la costa fuente. [Seguimiento v0.22](docs/VALIDATION-v0.22-FOLLOWUP.md).

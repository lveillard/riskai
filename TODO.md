...
La revisión estática de ronda 1 no cuenta comprobaciones: los tests planificados,
contadores, builds y evidencia final los cerrará Root.

- [x] Corregir arrastre derecho en pausa, conservar rueda de cámara y cancelar gestos pendientes al pausar/reanudar. La implementación queda lista; validación actual y build pendientes del cierre.
- [x] Selección rectangular sólo de edificios propios, incluidos puertos; Shift conserva la selección al arrastrar vacío. La implementación queda lista; validación actual pendiente.
- [ ] Verificar rueda del ratón dentro de paneles desplazables en el player real. La regresión sintética v0.21 queda planificada; la comprobación con el dispositivo físico sigue pendiente.
- [ ] Mostrar iconos de fragata y transporte en compras y colas. Mantener la explicación sólo en tooltip por hover; el texto permanente se rechaza y la ayuda por pulsación larga queda para fase 2. Catálogo regular y cola naval corregidos en código; validación visual/player y exportación final pendientes.
- [ ] Un solo defensor y propietario por puerto; el barco guardián habilita producción mediante los mismos comandos que el resto. Se conserva la prioridad de un guardián vivo y las anclas duales; añadir el escaneo `Contested` del guardián naval y validar sucesión/guardia en ejecución.
...
- [ ] Ampliar ligeramente el margen de relevo de guarnición, especialmente para caballeros; misma tolerancia terrestre y naval (2,0 frente a radio pintado 1,55). Añadir prueba de salida del guardián con separación realista; no ampliar el radio naval sin esa evidencia.
...
- [x] Evitar que la IA reenvíe tropas a ciudades inaccesibles por tierra. El filtro por tropa queda implementado; corregir el cursor para soportar cambios de roster y documentar/decidir aparte el umbral de ola mínima. Validación actual pendiente.
- [ ] IA con expediciones marítimas reales: comprar transporte, reunir tropas móviles, embarcar, navegar, desembarcar en una zona válida y atacar/capturar. Corregir conectividad de la fuente, equidad del pase de recuperación, caché/precálculo de atraques y conservación del ejército terrestre durante la espera; sin teletransportar carga. [Auditorías navales](docs/audits/NAVAL-v0.21.md) · [Revisión de ronda 1](docs/audits/REVIEW-v0.21.md) · [Revisión Grok](docs/audits/GROK-v0.21.md).
- [ ] Costa más natural: playas de desembarco, tramos rocosos y verdes no embarcables, con clasificación compartida entre representación y reglas. El horizonte debe recibir UV1; el test de orilla ya separa terreno fuente de la aserción de política. Geometría visual y alineación arena pintada/validada siguen abiertas; no elegir política todavía.
- [ ] Investigar retraso de órdenes en partida avanzada sin cerrar la sesión. La sonda de presupuesto sigue en 500 hasta tener medición reproducible; caché/precálculo de atraques aceptado para corregir picos de planificación. Picos navales, carga sostenida de 800 y comportamiento ARM pendientes. Medidas v0.20: unas 500 unidades, envío→aplicación 27–36 ms, una muestra de primer movimiento 1,24 s y rutas pendientes hasta 800 ms. Separar navegación de picos de frame de 23–26 s antes de atribuirlos a simulación o renderizado; la sesión v0.20 ya terminó.

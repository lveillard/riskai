...
namespace RiskAI
{
    ...
    public sealed class RuntimeVisualCapture : MonoBehaviour
    {
        ...
        IEnumerator Start()
        {
            ...
            ScreenCapture.CaptureScreenshot(Path.Combine(directory,"v09-player-overview.png"));
            yield return new WaitForSecondsRealtime(.6f);
            input.CameraRig.ZoomAt(2,new Vector2(Screen.width*.5f,Screen.height*.5f));
            yield return new WaitForSecondsRealtime(.7f);
            input.FocusHome();
            yield return new WaitForSecondsRealtime(1);
            ScreenCapture.CaptureScreenshot(Path.Combine(directory,"v09-player-city.png"));
            yield return new WaitForSecondsRealtime(.7f);
            input.CameraRig.Focus(MapLayout.Point(26*MapLayout.Spacing,3*MapLayout.Spacing));input.SelectAll();
            yield return new WaitForSecondsRealtime(1);
            ScreenCapture.CaptureScreenshot(Path.Combine(directory,"v09-player-highlands.png"));
            yield return new WaitForSecondsRealtime(.7f);
            input.FocusHarbor();
            yield return new WaitForSecondsRealtime(1);ScreenCapture.CaptureScreenshot(Path.Combine(directory,"v09-player-harbor.png"));
            yield return new WaitForSecondsRealtime(.7f);
            input.SelectFleet();input.FocusFleet();
            yield return new WaitForSecondsRealtime(1);ScreenCapture.CaptureScreenshot(Path.Combine(directory,"v09-player-fleet.png"));
            ...
            yield return new WaitForSecondsRealtime(3);ScreenCapture.CaptureScreenshot(Path.Combine(directory,"v09-player-river.png"));
            ...
            yield return new WaitForSecondsRealtime(3);ScreenCapture.CaptureScreenshot(Path.Combine(directory,"v09-player-island.png"));
            yield return new WaitForSecondsRealtime(.7f);input.HelpVisible=true;
            for(int i=0;i<3;i++)yield return null;
            ScreenCapture.CaptureScreenshot(Path.Combine(directory,"v09-player-help.png"));
            yield return new WaitForSecondsRealtime(.7f);
            bool captured=true;foreach(string name in new[]{"overview","city","highlands","harbor","fleet","river","island","help"})captured&=File.GetLastWriteTimeUtc(Path.Combine(directory,"v09-player-"+name+".png"))>=started;
            if(captured)

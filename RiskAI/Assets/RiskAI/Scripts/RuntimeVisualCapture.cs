...
namespace RiskAI
{
    ...
    public sealed class RuntimeVisualCapture : MonoBehaviour
    {
        ...
        IEnumerator CaptureImported(BattleSession battle,RtsController input)
        {
            ...
            input.CameraRig.ResetView();input.FocusHome();
            var home=input.SelectedTown;
            if(home)
            {
                var point=home.ClaimPoint+new Vector3(4,0,-2);
                var knight=battle.Spawn(0,Core.UnitKind.Guard,point);
                if(knight)
                {
                    input.SelectOnly(knight);input.Focus(knight.transform.position);
            ...
            {
                var transport=battle.Naval.Spawn(0,ShipKind.Transport,port.Berth);

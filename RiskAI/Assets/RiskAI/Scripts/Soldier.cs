...
namespace RiskAI
{
    ...
    public sealed class Soldier : CombatTarget
    {
        ...
        public bool IsIdle => isActiveAndEnabled && mode == OrderMode.Idle && !target && Agent && Agent.enabled && !Agent.hasPath;
        public NavMeshAgent Agent { get; private set; }
        public CombatTarget CurrentTarget => target;
        public string OrderLabel => target ? "En combate" : mode == OrderMode.Move ? "Moviendo" : mode == OrderMode.AttackMove ? "Avanzando y atacando" : mode == OrderMode.Hold ? "Manteniendo posición" : mode == OrderMode.Patrol ? "Patrullando" : mode == OrderMode.Follow ? "Siguiendo" : "Preparado";
        ...
        {
            session = battle; Team = team; Kind = kind; Health = MaxHealth; anchor = transform.position;
            Agent = GetComponent<NavMeshAgent>(); Agent.radius = .24f; Agent.height = 1.3f;
            Agent.speed = BattleRules.Speed(kind); Agent.acceleration = 32; Agent.angularSpeed = 540;
            Agent.stoppingDistance = .15f; Agent.autoBraking = true;
            Agent.obstacleAvoidanceType = ObstacleAvoidanceType.HighQualityObstacleAvoidance;
            Agent.avoidancePriority = 25 + (GetInstanceID() & 31);
            ...
            if(kind==UnitKind.Medic)gameObject.AddComponent<MedicSupport>().Initialize(this,battle);
            visualAnimator=GetComponent<SoldierAnimator>();
            ring = VisualFactory.Ring(transform, .43f, .045f, new Color(.5f, 1f, .6f)); ring.enabled = false;
            nextSense = Time.time + Random.value * .15f;
        }
        ...
        {
            if (!Agent || !Agent.isOnNavMesh || !NavMesh.SamplePosition(point, out var hit, 8, NavMesh.AllAreas)) return;
        ...
        void Acquire()
        {
            ...
            foreach (var enemy in session.Targets)
            {
                ...
                int pressure = 0;
                foreach (var ally in session.Units) if (ally && ally.Team == Team && ally.CurrentTarget == enemy) pressure++;
                float candidate = distance + (Kind == UnitKind.Footman ? pressure * .48f : pressure * .1f);
                if (candidate < score) { best = enemy; score = candidate; }
            }
            if (best) SetTarget(best);
        }
        void Update()
        {
            if (!session || session.Paused || session.Winner >= 0 || !Agent || !Agent.isOnNavMesh) return;
            if (strikeAt >= 0 && Time.time >= strikeAt)
            {
                if (strikeTarget && strikeTarget.Health > 0 && Vector3.Distance(transform.position, strikeTarget.ApproachPoint(transform.position)) <= BattleRules.Range(Kind) + .55f && Vector3.Distance(transform.position,strikeTarget.ApproachPoint(transform.position))>=BattleRules.MinimumRange(Kind) && Visible(strikeTarget))
                {
                    ...
                    if (BattleRules.Ranged(Kind)) VisualFactory.Arrow(AimPoint, strikeTarget.AimPoint, strikeTarget, damage, Team, this, AttackType);
            ...
            if (!target && Time.time >= nextSense) { nextSense = Time.time + .18f; Acquire(); }
            if (target) Fight(); else Travel();
            wasFighting = target != null;
            float stride = Agent.velocity.magnitude > .15f ? Mathf.Sin(Time.time * 13 + GetInstanceID()) * 30 : 0;
        ...
        void Fight()
        {
            ...
            if(distance<BattleRules.MinimumRange(Kind))
            {
                ...
                {
                    nextPath=Time.time+.4f;var away=transform.position-target.transform.position;away.y=0;
            ...
            if (distance <= BattleRules.Range(Kind) && Visible(target))
            {
                ...
                if (direction.sqrMagnitude > .001f) transform.rotation = Quaternion.RotateTowards(transform.rotation, Quaternion.LookRotation(direction), 650 * Time.deltaTime);
                if (Time.time >= nextAttack && strikeAt < 0)
                {
                    nextAttack = Time.time + BattleRules.AttackInterval(Kind); strikeAt = Time.time + (Kind == UnitKind.Footman ? .17f : .24f);
            ...
            else if (strikeAt < 0 && Time.time >= nextPath)
            {
                nextPath = Time.time + .16f; Agent.isStopped = false;
        ...
        void Travel()
        {
            ...
            if (mode == OrderMode.Follow)
            {
                ...
                if (Time.time >= nextPath) { nextPath = Time.time + .2f; Agent.stoppingDistance = 2; Agent.SetDestination(followTarget.transform.position); }
            ...
            if (mode == OrderMode.Move || mode == OrderMode.AttackMove || mode == OrderMode.Patrol)
            {
                ...
                stalled = Agent.velocity.sqrMagnitude < .04f ? stalled + Time.deltaTime : 0;
                if (distance < .65f || (Agent.hasPath && Agent.remainingDistance < .4f) || (stalled > 1.2f && distance < 3)) Complete();
                else if (!Agent.hasPath && Time.time >= nextPath) { Agent.stoppingDistance = .15f; Agent.SetDestination(destination); nextPath = Time.time + .5f; }
            }
        }
        ...
        public void DestroyEmbarked(int attacker)
        {
            ...
            if(attacker>=0&&attacker<2){session.Kills[attacker]++;session.Economy.Gold[attacker]+=2;}
            session.Units.Remove(this);session.Targets.Remove(this);Destroy(gameObject);
        }
        public override void TakeDamage(float damage, int attacker, CombatTarget source = null)
        {
            ...
            {
                if (attacker >= 0 && attacker < 2) { session.Kills[attacker]++; session.Economy.Gold[attacker]+=2; }
                session.Units.Remove(this);session.Targets.Remove(this);Select(false);Agent.enabled=false;GetComponent<Collider>().enabled=false;enabled=false;
                if(visualAnimator){visualAnimator.Die();Destroy(gameObject,1.4f);}else Destroy(gameObject);
                return;
            }
            if (source && mode != OrderMode.Move && mode != OrderMode.Hold && mode != OrderMode.Follow && !target)
                SetTarget(source);
            if (source) foreach (var ally in session.Units)
                if (ally && ally.Team == Team && ally.IsIdle && Vector3.Distance(transform.position, ally.transform.position) < 5)

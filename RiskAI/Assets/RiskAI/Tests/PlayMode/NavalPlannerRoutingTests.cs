...
namespace RiskAI.Tests
{
    public sealed class NavalPlannerRoutingTests
    {
        ...
        public IEnumerator ExistingTransportChoosesOnlySourceInItsSeaComponent()
        {
            ...
            var transport=naval.Spawn(1,UnitKind.Transport,transportPoint);
        ...
        public IEnumerator PartiallyBoardedWaveAttacksOnlyTheActualCargo()
        {
            ...
            var transport=BattleTestScenario.Ship(naval,1,UnitKind.Transport,berth);
        ...
        public IEnumerator IsolatedEmptyBoatAllowsPaidLocalTransportAndWaitsForThatBoat()
        {
            ...
            var isolated=BattleTestScenario.Ship(naval,1,UnitKind.Transport,new Vector3(-20,-.24f,0));
            Assert.That(SeaNavigation.AreConnected(isolated.transform.position,homeBerth),Is.False);
            int gold=UnitCatalog.Get(UnitKind.Transport).Cost+naval.FirstFleetSavingsTargetFor(1);battle.Economy.Gold[1]=gold;
            ...
            Assert.That(battle.Economy.Gold[1],Is.EqualTo(gold-UnitCatalog.Get(UnitKind.Transport).Cost));
            ...
            home.SimTick(UnitCatalog.Get(UnitKind.Transport).TrainSeconds+.1f);
            var local=naval.Ships.FirstOrDefault(s=>s&&s.Team==1&&s.Kind==UnitKind.Transport&&s!=isolated);

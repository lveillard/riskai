...
namespace RiskAI.Tests
{
    public sealed class CommanderConnectivityTests
    {
        ...
        [UnityTest]
        public IEnumerator UnreachableIslandObjectivesNeverReceiveLandOrdersAcrossRosterChurn()
        {
            var home=battle.Towns.First(town=>town.State.Owner==1);
            ...
            var mainland=battle.Towns.First(town=>!OnIsland(town.ClaimPoint)&&town.State.Country!=home.State.Country);
            foreach(var town in battle.Towns)town.State.Owner=town==home?1:0;
            // The islands would complete the home country, but need a transport.
            ...
            yield return null;
            yield return WaitForPaths(force);
            var commanded=force.Where(unit=>!unit.IsIdle).ToArray();
            Assert.That(commanded.Length,Is.GreaterThanOrEqualTo(2),"The reachable objective receives a wave.");
            AssertOnlyReachableOrders(commanded,mainland,firstIsland,secondIsland);

            // Roster churn between decisions must neither rewind onto the islands
            // nor leave fresh troops without a reachable order.
            force[12].gameObject.SetActive(false);
            var replacement=BattleTestScenario.Mobile(battle,1,UnitKind.Footman,Sample(mainland.ClaimPoint));
            InvokeOffense(false);battle.Commands.Tick();
            yield return null;
            var all=force.Take(12).Concat(new[]{replacement}).ToArray();
            yield return WaitForPaths(all);
            AssertOnlyReachableOrders(all.Where(unit=>!unit.IsIdle).ToArray(),mainland,firstIsland,secondIsland);
            Assert.That(replacement.IsIdle,Is.False,"A fresh recruit beside an attacking wave joins it.");
        }

        IEnumerator WaitForPaths(Soldier[] units)
        {
            float deadline=Time.realtimeSinceStartup+2;
            while(units.Any(unit=>unit&&unit.isActiveAndEnabled&&unit.Agent.pathPending)&&Time.realtimeSinceStartup<deadline)yield return null;
        }

        static void AssertOnlyReachableOrders(Soldier[] units,Settlement mainland,params Settlement[] islands)
        {
            foreach(var unit in units)
            {
                Assert.That(unit.Agent.pathStatus,Is.EqualTo(NavMeshPathStatus.PathComplete));
                Assert.That(Vector3.Distance(unit.Agent.destination,mainland.ClaimPoint),Is.LessThan(5f),"The reachable mainland objective receives the formation.");
                foreach(var island in islands)Assert.That(Vector3.Distance(unit.Agent.destination,island.ClaimPoint),Is.GreaterThan(10f));
            }
        }

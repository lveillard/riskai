...
namespace RiskAI.Tests
{
    ...
    public sealed class SourceRosterV030Tests
    {
            ...
            int dice, int sides, float range, float cooldown, float speed, float armor, AttackKind attack, ArmorKind defense)
        {
            ...
            Assert.That(BattleRules.Cost(kind), Is.EqualTo(gold));
            Assert.That(BattleRules.PointValue(kind), Is.EqualTo(gold), "upoi equals ugol for every v0.30 land unit.");
            Assert.That(profile.Health, Is.EqualTo(health));
            Assert.That(profile.BaseDamage, Is.EqualTo(baseDamage));
            Assert.That(profile.Dice, Is.EqualTo(dice));
            Assert.That(profile.Sides, Is.EqualTo(sides));
            Assert.That(profile.Range, Is.EqualTo(range));
            ...
            Assert.That(BattleRules.Ranged(kind), Is.EqualTo(range > 2));
        }
        ...
        {
            var artillery = SourceWeapons.For(UnitKind.Artillery, AttackKind.Piercing);
            ...
            Assert.That(SourceWeapons.AcquisitionRange(UnitKind.Artillery), Is.EqualTo(20));
            Assert.That(BattleRules.Mechanical(UnitKind.Artillery), Is.True);
            Assert.That(BattleRules.MinimumRange(UnitKind.Artillery), Is.Zero, "hmtt has no minimum range.");

            var tank = SourceWeapons.For(UnitKind.Tank, AttackKind.Siege);
            ...
            Assert.That(BattleRules.Mechanical(UnitKind.Tank), Is.True);

            Assert.That(SourceWeapons.For(UnitKind.EliteRifleman, AttackKind.Piercing).IsProjectile, Is.False);
            Assert.That(SourceWeapons.For(UnitKind.Roarer, AttackKind.Piercing).ProjectileSpeed, Is.EqualTo(900f / 50f));
            Assert.That(SourceWeapons.AcquisitionRange(UnitKind.Roarer), Is.EqualTo(8));
            Assert.That(SourceWeapons.AcquisitionRange(UnitKind.EliteRifleman), Is.EqualTo(12));
            Assert.That(SourceWeapons.AcquisitionRange(UnitKind.ArmyGeneral), Is.EqualTo(10));
            Assert.That(BattleRules.Mechanical(UnitKind.Medic), Is.False);
        }

        [Test]
        public void MortarMatchesH00HAndHmtmSource()
        {
            var profile = BattleRules.Profile(UnitKind.Mortar);
            var weapon = SourceWeapons.For(UnitKind.Mortar, profile.Attack);
            Assert.That(profile.Attack, Is.EqualTo(AttackKind.Siege));
            Assert.That(profile.Defense, Is.EqualTo(ArmorKind.Medium), "RoC hmtm defType=medium (runtime baseline).");
            Assert.That(profile.Speed, Is.EqualTo(230f / 50f).Within(.0001f));
            Assert.That(profile.Cooldown, Is.EqualTo(3.5f));
            Assert.That(profile.AttackPoint, Is.EqualTo(1f));
            Assert.That(BattleRules.MinimumRange(UnitKind.Mortar), Is.EqualTo(250f / 50f));
            Assert.That(SourceWeapons.AcquisitionRange(UnitKind.Mortar), Is.EqualTo(900f / 50f));
        ...
        {
            Assert.That(BattleRules.Health(UnitKind.Medic), Is.EqualTo(220f), "Local v0.30 product decision (h00E explicit uhpm=250).");
            var mana = SupportAbilities.Mana(UnitKind.Medic);
            ...
            Assert.That(SupportAbilities.HealAmount, Is.EqualTo(25f));
            Assert.That(SupportAbilities.HealManaCost, Is.EqualTo(5f));
            Assert.That(SupportAbilities.HealCooldown, Is.EqualTo(1f));
            Assert.That(SupportAbilities.HealRange, Is.EqualTo(250f / 50f));
        }
        ...
        public void ManaPoolRegeneratesClampsAndRefusesUnaffordableCasts()
        {
            ...
            pool.Reset(SupportAbilities.Mana(UnitKind.Medic));
            int heals = 0;
            while (pool.TrySpend(SupportAbilities.HealManaCost)) heals++;
            Assert.That(heals, Is.EqualTo(15), "75 initial mana pays fifteen 5-mana heals.");
            Assert.That(pool.CanSpend(SupportAbilities.HealManaCost), Is.False);
            pool.Tick(2f);
            ...
            none.Reset(SupportAbilities.Mana(UnitKind.Archer));
        ...
        {
            Assert.That(SupportAbilities.CanRoar(UnitKind.Roarer), Is.True);
            Assert.That(SupportAbilities.CanRoar(UnitKind.ArmyGeneral), Is.True, "h00J also attaches Aroa.");
            Assert.That(SupportAbilities.CanRoar(UnitKind.Medic), Is.False);
            Assert.That(SupportAbilities.RoarArea, Is.EqualTo(700f / 50f), "war3map.w3a Aroa aare=700.");
            Assert.That(SupportAbilities.RoarManaCost, Is.EqualTo(100f));
            Assert.That(SupportAbilities.RoarDuration, Is.EqualTo(45f));
            Assert.That(SupportAbilities.DamageMultiplier(true), Is.EqualTo(1.25f));
            Assert.That(SupportAbilities.DamageMultiplier(false), Is.EqualTo(1f));
            var roarer = SupportAbilities.Mana(UnitKind.Roarer);
            ...
            var general = SupportAbilities.Mana(UnitKind.ArmyGeneral);

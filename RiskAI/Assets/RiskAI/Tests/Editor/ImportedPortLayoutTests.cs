...
namespace RiskAI.Tests
{
    public sealed class ImportedPortLayoutTests
    {
        ...
        public void ShoulderAnchorsPreserveSourceAnchorsForDetachedVariants(ScenarioMap scenario)
        {
        ...
        public void SourceCenteredIntegratedTowerCoversOnlyItsOwnOpeningPost(ScenarioMap scenario)
        {
            ...
            try
            {
                ...
                foreach(var city in MapLayout.Towns)
                {
                    ...
                    Assert.That(own,Is.LessThan(UnitCatalog.Tower.TownWeapon.Range),city.Id+" tower covers its own source guard");
                    foreach(var other in MapLayout.Towns)
                    {
                        if(other.Id==city.Id)continue;
                        Assert.That(FlatDistance(city.Position,other.ClaimPoint),Is.GreaterThan(UnitCatalog.Tower.TownWeapon.Range),

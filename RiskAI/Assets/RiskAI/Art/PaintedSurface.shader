 ...
 SubShader
 {
  ...
  Pass
  {
   ...
   half4 Frag(V i):SV_Target
   {
    ...
    // Recolour (faction roofs): the dark tile only supplies relief. Mapping its
    // luminance into a narrow bright band keeps the player hue at a canonical value
    // instead of pushing orange to brown and red toward maroon.
    half relief=lerp(.5,1.05,saturate(sqrt(dot(c,half3(.2126,.7152,.0722)))*4.5));
    c=lerp(c*_Tint.rgb,relief*_Tint.rgb,_Recolor);

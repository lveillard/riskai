Shader "RiskAI/RiverWater"
{
 SubShader
 {
  ...
  Pass
  {
   ...
   half4 Frag(V i):SV_Target{half4 c=RiskWater(i.w,i.p,float3(0,1,0),0,0);half horizon=RiskHorizonFade(i.w.xz);c.rgb=lerp(c.rgb,_RiskHorizonColor.rgb,horizon);c.a=max(c.a,horizon);c.rgb=MixFog(c.rgb,i.fog);return c;}
   ENDHLSL
  }
 }
}

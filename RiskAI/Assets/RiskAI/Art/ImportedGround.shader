Shader "RiskAI/ImportedGround"
{
 ...
 SubShader
 {
  ...
  Pass
  {
   ...
   // Linear averages of the atlas tiles used below (measured from the PNGs).
   static const half3 RiskAverageGrass=half3(.0158,.0250,.0179),RiskAverageDry=half3(.1695,.1281,.0437);
   static const half3 RiskAverageSand=half3(.3682,.2523,.1034),RiskAveragePale=half3(.3138,.2922,.2476),RiskAverageRock=half3(.1215,.0925,.0560);
   half3 BiomeLand(half3 green,half3 dryGrass,half3 pale,half3 sandTile,half3 rock,half arid,half cold,half4 biome,half patch,half biomeNoise,half biomeGrain,half scrubNoise,half rockNoise)
   {
    ...
    scrub=lerp(scrub,pale*half3(.78,.70,.55),smoothstep(.56,.8,scrubNoise)*.65);
    ...
    half rocky=biome.a*smoothstep(.30,.72,rockNoise);
    return lerp(land,lerp(rock*1.2,pale*.85,.45),rocky*.72);
   }
   half4 Frag(V i):SV_Target
   {
    ...
    // Satellite colour grading (NASA Blue Marble, georeferenced per map): the
    // texture blend keeps only its local detail, measured against the same blend
    // of the atlas' average colours, so no source tile or country border shows.
    half4 ground=RiskGroundColor(i.w.xz);
    half groundStrength=saturate(_RiskGroundGrid.w);
    half3 reference=BiomeLand(RiskAverageGrass*1.65,RiskAverageDry,RiskAveragePale,RiskAverageSand,RiskAverageRock,arid,cold,biome,patch,biomeNoise,biomeGrain,scrubNoise,rockNoise);
    half detail=clamp(dot(land,half3(.30,.59,.11))/max(dot(reference,half3(.30,.59,.11)),.004),.55,1.7);
    land=lerp(land,ground.rgb*_RiskGroundGrid.z*lerp(1,detail,.85),groundStrength*.92);
    half departure=max(groundStrength,biomeStrength*saturate(max(smoothstep(.16,.5,arid),smoothstep(.3,.62,cold))));
    half tileLuma=dot(i.color.rgb,half3(.30,.59,.11));
    // Source tiles are painted per country with straight borders; with satellite
    // colour they no longer tint the ground at all.
    half3 tint=lerp(lerp(i.color.rgb,(.5+.5*tileLuma/.62).xxx,departure),1,groundStrength);
    ...
    // The clamped coast field would streak outward; shore materials fade out beyond the edge.
    coast*=1-smoothstep(4,24,RiskOutsidePlayable(i.w.xz));
    ...
    color*=lerp(lerp(half3(.84,.92,.80),half3(.90,.88,.86),departure),lerp(half3(1.10,1.07,.96),half3(1.06,1.05,1.03),groundStrength),patch);
    color*=.90+meadow*.17;
